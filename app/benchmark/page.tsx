              ...
              <div key={model}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}>
                  ...
                  <span style={{ fontSize: "0.8125rem", color: "var(--text-faint)" }}>
                    {t(stats.recipes === 1 ? "benchmark.recipeCount_one" : "benchmark.recipeCount_other", { count: stats.recipes })}
                  </span>
                </div>
                ...
                <div key={model} className="card" style={{ padding: "20px" }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                    <div>
                      ...
                      <p style={{ fontSize: "0.75rem", color: "var(--text-faint)" }}>{t("benchmark.recipes")}</p>
                    </div>
                    <div>
                      <p style={{ fontSize: "1.5rem", fontWeight: 700, fontFamily: "var(--font-playfair), serif" }}>{stats.reviews}</p>
                      <p style={{ fontSize: "0.75rem", color: "var(--text-faint)" }}>{t("benchmark.reviews")}</p>
                    </div>
                    <div>
                      <p style={{ fontSize: "1.25rem", fontWeight: 600, color: "var(--text)" }}>
                        {stats.avgTaste > 0 ? `★ ${stats.avgTaste.toFixed(1)}` : "—"}
                      </p>
                      <p style={{ fontSize: "0.75rem", color: "var(--text-faint)" }}>{t("benchmark.avgTaste")}</p>
                    </div>
                    <div>
                      <p style={{ fontSize: "1.25rem", fontWeight: 600, color: "var(--text)" }}>
                        {stats.avgAccuracy > 0 ? `★ ${stats.avgAccuracy.toFixed(1)}` : "—"}
                      </p>
                      <p style={{ fontSize: "0.75rem", color: "var(--text-faint)" }}>{t("benchmark.avgAccuracy")}</p>
                    </div>
                  </div>
                  {stats.reviews > 0 && (
                    <div style={{ marginTop: "12px", paddingTop: "12px", borderTop: "1px solid var(--border)" }}>
                      <p style={{ fontSize: "0.75rem", color: "var(--text-faint)" }}>
                        {t("benchmark.goodRatings", { taste: stats.pctGoodTaste.toFixed(0), accuracy: stats.pctGoodAccuracy.toFixed(0) })}
                      </p>
                    </div>
                  )}
                </div>
            ...
            {Object.entries(data).map(([model, stats]) => (
              <div key={model} style={{ marginBottom: "40px" }}>
                      ...
                      <div>
                        ...
                        {b.created_at && (
                          <p style={{ fontSize: "0.75rem", color: "var(--text-faint)", marginTop: "2px" }}>
                            {new Date(b.created_at).toLocaleDateString(locale === "zh" ? "zh-CN" : "en-US", { year: "numeric", month: "short", day: "numeric" })}
                          </p>
                        )}
                      </div>
                      <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
                        {b.accuracy_rating && (
                          <span style={{ fontSize: "0.75rem", color: "var(--text-muted)" }}>
                            {t("recipe.accuracyRating")}: ★{b.accuracy_rating}
                          </span>
                        )}
                        {b.rating && (
                ...
                {stats.branches.length === 0 && (
                  <p style={{ color: "var(--text-faint)", fontSize: "0.875rem", fontStyle: "italic" }}>{t("recipe.noReviews")}</p>
                )}
              </div>
            ))}
              ...
              {(() => {
                ...
                return (
                  <ul style={{ fontSize: "0.875rem", color: "var(--text-muted)", lineHeight: 1.8, paddingLeft: "20px", margin: 0 }}>
                    <li><strong>{topModel}</strong>{t("benchmark.mostRecipesSuffix", { count: sorted[0][1].recipes })}</li>
                    {topAccuracy && (
                      <li><strong>{topAccuracy[0]}</strong>{t("benchmark.highestAccuracySuffix", { rating: topAccuracy[1].avgAccuracy.toFixed(1) })}</li>
                    )}
                    <li>{t("benchmark.smallDataset", { reviews: totalReviews, models: totalModels })}</li>
                  </ul>
                );
              })()}
            </div>
