  ...
  return (
    <Link href={`/recipes/${recipe.id}`} className="card group block" style={{ textDecoration: 'none' }}>
      ...
      <div style={{ padding: '20px' }}>
        {recipe.bread_type && (
          <p className="section-label" style={{ marginBottom: '6px' }}>{recipe.bread_type}</p>
        )}
        <h3 style={{
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    ]).then(([{ data: recipesData, error }, { data: reviewsData }]) => {
      if (!error && recipesData) {
        const reviewsMap: Record<string, { sum: number; count: number }> = {};
        (reviewsData || []).forEach((r: any) => {
          if (!reviewsMap[r.recipe_id]) reviewsMap[r.recipe_id] = { sum: 0, count: 0 };
          reviewsMap[r.recipe_id].sum += r.rating;
          reviewsMap[r.recipe_id].count += 1;
        });
        setRecipes(recipesData.map((r: any) => {
          const stats = reviewsMap[r.id] || { sum: 0, count: 0 };
          return {
            ...r,
            avg_rating: stats.count ? stats.sum / stats.count : 0,
